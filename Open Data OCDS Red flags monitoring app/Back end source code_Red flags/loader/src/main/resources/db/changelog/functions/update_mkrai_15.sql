...
CREATE OR REPLACE FUNCTION update_mkrai_15()
    RETURNS VOID AS
$$
DECLARE
    _tender                RECORD;
...
BEGIN
    ...
    FOR _tender IN SELECT *
        ...
        LOOP
            ...
            THEN
                INSERT INTO tender_indicator VALUES (_tender.id, _INDICATOR_ID, INCORRECT, now());
                CONTINUE;
            END IF;

            SELECT * FROM tender WHERE outer_id = _related_process.identifier INTO _previous_tender;

            IF _previous_tender.procurement_method_details NOT IN ('oneStage', 'downgrade', 'simplicated') OR
               _previous_tender.status_details NOT IN ('contractSigned', 'evaluationComplete')
            THEN
                INSERT INTO tender_indicator VALUES (_tender.id, _INDICATOR_ID, PASSED, now());
                CONTINUE;
            END IF;
            ...
            IF _current_amount / _previous_amount * 100 > 5
            THEN
