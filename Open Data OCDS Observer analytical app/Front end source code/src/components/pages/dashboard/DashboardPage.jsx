...
import {
  getIndicatorsData,
  clearIndicatorsData,
} from '../../../store/dashboard/indicators/indicatorsActions'
...
import { HORIZONTAL_BAR_CHART_SECONDARY_COLORS, DAY_NAMES } from '../../pageChart/constants'
...
export const getDayTranslate = (day) => {
  ...
  return dayName ? dayName.en : null
}
...
class Dashboard extends PureComponent {
  ...
  constructor(props) {
    ...
    // _.isEmpty(props.indicatorsData) && this.props.getIndicatorsData()
    this.state = {
  ...
  componentDidMount() {
    // this.onMouseMove()
    this.props.getIndicatorsData()
    //   .then(() => {
    //   !_.isEmpty(this.props.indicatorsData) && this.setState({
    //     // activeDays: this.props.indicatorsData.activeDays,
    //     activeDays: _.orderBy(this.props.indicatorsData.activeDays, [], ['asc']),
    //   })
    // })
  }

  componentWillUnmount() {
    this.props.clearIndicatorsData()
  }
  ...
  getProceduresTableColumns = () => {
    ...
    const TOP_FIVE_PROCUREMENT_COLUMNS = [
      ...
      {
        // Header: intl.formatMessage({ id: 'common.home.text.0.09' }),
        Header: intl.formatMessage({ id: 'common.home.text.0.10.1' }),
    ...
    if (!_.isEmpty(titleIsEmpty)) {
      ...
      columnsConfig.unshift({ Header: '', accessor: 'procurementNumber' })
    }
  ...
  renderMiniKPIs = () => {
    ...
    // />
    if (_.isEmpty(indicatorsData)) return <Loader
      isActive={_.isEmpty(indicatorsData)}
    />
    return <React.Fragment>
      <Card className="col-md-12 col-xl-4 home-cards dark-theme" cardClass="d-flex flex-row mini-kpi h-100">
        <div className="flex-container">
          {this.renderMiniKPIValue(indicatorsData.buyersCount, 'color-madang')}
        </div>
        {/*<Divider borderBottom={0} borderLeft={'1px solid #E9EDF2'} marginLeft={15} width={1} />*/}
        <span className="home-cards-text margin-left-15"><FormattedMessage
          id="common.home.kpi.numberOfCustomers.label" /></span>
      </Card>
      <Card className="col-md-12 col-xl-4 home-cards dark-theme" cardClass="d-flex flex-row mini-kpi h-100">
        <div className="flex-container">
          {this.renderMiniKPIValue(indicatorsData.suppliersCount, 'color-perano')}
        </div>
        {/*<Divider borderBottom={0} borderLeft={'1px solid #E9EDF2'} marginLeft={15} width={1} />*/}
        <span className="home-cards-text margin-left-15"><FormattedMessage
          id="common.home.kpi.numberOfSuppliers.label" /></span>
      </Card>
      <Card className="col-md-12 col-xl-4 home-cards dark-theme" cardClass="d-flex flex-row mini-kpi h-100">
        <div className="flex-container">
          {this.renderMiniKPIValue(indicatorsData.completedLotsCount, 'color-madang')}
        </div>
        {/*<Divider borderBottom={0} borderLeft={'1px solid #E9EDF2'} marginLeft={15} width={1} />*/}
        <span className="home-cards-text margin-left-15"><FormattedMessage
          id="common.home.kpi.numberOfSuccessfullyLots.label" /></span>
      </Card>
    </React.Fragment>
  }
  ...
  renderMiniCharts = () => {
    ...
    const MINI_CHART_CONFIG = [
      {
        name: <FormattedMessage id="page.common.text.5" />,
        unit: intl.formatMessage({ id: 'common.psc.text' }),
        colorPreset: 'c-preset-dark-blue-mono',
        localeId: 'common.home.kpiChart.averageNumberLots.label',
        format: 'quantity',
      },
      {
        name: <FormattedMessage id="page.common.text.6" />,
        unit: intl.formatMessage({ id: 'common.psc.text' }),
      ...
      {
        name: <FormattedMessage id="page.common.text.7" />,
        unit: intl.formatMessage({ id: 'common.byn.text' }),
        // colorPreset: 'c-preset-blue-mono',
        colorPreset: 'c-preset-dark-blue-mono',
        localeId: 'page.common.text.7',
        format: 'currency',
      },
    ]
    return <React.Fragment>
      {
        // !_.isEmpty(indicatorsData.avgLotsCountByDate) &&
        indicatorsData.hasOwnProperty('avgLotsCountByDate') &&
        <GradientChartCard
          data={indicatorsData.avgLotsCountByDate}
          amount={indicatorsData.avgLotsCount}
          config={MINI_CHART_CONFIG[0]}
          isEmpty={_.isEmpty(indicatorsData.avgLotsCountByDate)}
          intl={intl}
          lang={this.props.lang}
          activeDays={indicatorsData.activeDays}
        />
      }
      {
        // !_.isEmpty(indicatorsData.avgCpvCountByDates) &&
        indicatorsData.hasOwnProperty('avgCpvCountByDates') &&
        <GradientChartCard
          data={indicatorsData.avgCpvCountByDates}
          amount={indicatorsData.avgCpvCount}
          // data={perProcedureOkrbCount}
          config={MINI_CHART_CONFIG[1]}
          isEmpty={_.isEmpty(indicatorsData.avgCpvCountByDates)}
          intl={intl}
          lang={this.props.lang}
          activeDays={indicatorsData.activeDays}
        />
      ...
      {
        // !_.isEmpty(indicatorsData.publishedLotsAmountByDate) &&
        indicatorsData.hasOwnProperty('publishedLotsAmountByDate') &&
        <GradientChartCard
          data={indicatorsData.publishedLotsAmountByDate}
          // amount={indicatorsData.publishedLotsCount}
          amount={indicatorsData.publishedLotsAmount}
          // data={perSupplierContractsCount}
          config={MINI_CHART_CONFIG[2]}
          isEmpty={_.isEmpty(indicatorsData.publishedLotsAmountByDate)}
          intl={intl}
          lang={this.props.lang}
          activeDays={indicatorsData.activeDays}
        />
  ...
  getProceduresTableData = (data) => {
    if (!this.props.indicatorsData.hasOwnProperty('topProcurements'))
      return []

    const { topProcurements } = this.props.indicatorsData
    const { intl } = this.context

    return _.map(topProcurements, (item) => {
      return {
        ...
        amount: intl.formatNumber((item.itemsAmount / 1000000).toFixed(2)),
      }
    })
  }
  ...
  getProceduresTopTableData = data => {
    ...
    let i18nCpv = _.find(this.props.translationI18nData.entries, { key: `procurementMethod.status.${data.procurementMethod}` })
  ...
  renderKpiContractsChart = (data, key, format, xAxisKey) => {
    ...
    let sortedActiveDays = _.orderBy(this.props.indicatorsData.activeDays, [], ['asc'])
    ...
    GradientChartConfig.config = {
      ...
      series: [
        {
          ...
          data: sortedActiveDays.map((dayName) => {
            let valueByDate = _.find(data, { date: dayName })
            return valueByDate ? [valueByDate.date, valueByDate[key]] : [dayName, 0]
          }),
          // data: data.map(item => {
      ...
      tooltip: {
        formatter: function () {
          return '<span style="font-size: 10px">' + this.key + '</span><br/><span style="font-size: 10px">' + this.point.y.toLocaleString('ru') + ' ' + intl.formatMessage({ id: 'common.psc.text' }) + '</span><br/>'
      ...
      xAxis: {
        ...
        //   // let dayNumber = new Date(item.date).getDay()
        //   return '<span>' + item.date + '<br /><span class="d-none d-md-block d-lg-block">' + intl.formatMessage({ id: `day.name.${DAY_NAMES[new Date(item.date).getDay()]}` }) + '</span></span>'
        // }),
        categories: sortedActiveDays.map(dayName => {
          // return `<span>${item.date}<br /><span class="d-none d-md-block d-lg-block">${this.props.lang === 'ru' ? item.day : getDayTranslate(item.day)}</span></span>`
          // let dayNumber = new Date(item.date).getDay()
          return '<span>' + dayName + '<br /><span class="d-none d-md-block d-lg-block">' + intl.formatMessage({ id: `day.name.${DAY_NAMES[new Date(dayName).getDay()]}` }) + '</span></span>'
        }),
        title: {
          text: '',
        },
      ...
      yAxis: {
        ...
        title: {
          // text: xAxisKey,
          text: intl.formatMessage({ id: 'page.chart.text.2' }),
          style: {
            color: '#AFAFAF',
            fontStyle: 'normal',
            fontWeight: 'normal',
    ...
    GradientChartConfig.config.lang.noData = intl.formatMessage({ id: GradientChartConfig.config.lang.noData })
  ...
  renderOkbrInfo = () => {
    const { indicatorsData, lang, translationI18nData } = this.props

    if (_.isEmpty(indicatorsData)) return

    let translateCpvName = !_.isEmpty(indicatorsData.top5CPV) ? _.find(translationI18nData.entries, { key: indicatorsData.top5CPV[0].name }) : ''
    return <div className="swith-card-content">
      <IconCart />
      <div className="swith-card-content-title"><FormattedMessage id="page.common.text.31" />
      </div>
      <div
        className="swith-card-content-value">{_.isEmpty(translateCpvName) ? translateCpvName : (lang === 'en' ? translateCpvName.en : translateCpvName.ru)}</div>
    </div>
  }

  renderProceduresTopInfo = () => {
    const { indicatorsData } = this.props
    this.props.indicatorsData.hasOwnProperty('tenderOfWeek')
    let amount = (indicatorsData.hasOwnProperty('tenderOfWeek') && !_.isEmpty(indicatorsData.tenderOfWeek)) ? indicatorsData.tenderOfWeek.amount : 0
    if (_.isEmpty(indicatorsData)) return
    ...
    return <div className="swith-card-content">
      ...
      <div className="swith-card-content-value">{numeral(amount).format('0.0 a')} <FormattedMessage
        id="common.byn.text" />
        {/*<br />*/}
  ...
  renderEnquiriesCountInfo = () => {
    ...
    return <div className="swith-card-content">
      ...
      <div
        className="swith-card-content-value">{indicatorsData.enquiriesCount ? indicatorsData.enquiriesCount : 0}</div>
    </div>
  }

  renderOkbrChart = () => {
    const { indicatorsData, translationI18nData, lang } = this.props
    ...
    OkbrChartConfig.config.tooltip = {
      // useHTML: true,
      formatter: function () {
        return '<span style="font-size: 14px">' + this.key + '</span><br/><span style="font-size: 14px"><b>' + numeral(this.point.y).format('0 a') + ' ' + intl.formatMessage({ id: 'common.byn.text' }) + '</b></span><br/>'
    ...
    OkbrChartConfig.config.xAxis = {
      ...
      tickInterval: 1,
      categories: indicatorsData.top5CPV.map((item, i) => {
        return i + 1
      }),
      title: {
        margin: 30,
        text: intl.formatMessage({ id: 'common.home.switchDefault.subjectMatters.label' }),
        style: {
          'fontWeight': 'normal',
          'fontSize': '16px',
          'lineHeight': '24px',
    ...
    // }
    OkbrChartConfig.config.series[0].data = indicatorsData.top5CPV.map((item, index) => {
      ...
      return {
        ...
        x: index,
        // name: item.name,
        name: _.find(translationI18nData.entries, { key: item.name })[lang === 'en' ? 'en' : 'ru'],
      }
    })
  ...
  renderEnquiriesCountChart = () => {
    const { intl } = this.context
    const { indicatorsData } = this.props
    // indicatorsData.enquiriesCountByDate
    if (_.isEmpty(indicatorsData)) return

    let sortedActiveDays = _.orderBy(this.props.indicatorsData.activeDays, [], ['asc'])

    let EnquiriesCountConfig = _.cloneDeep(ChartConfig)
    let sortedData = _.orderBy(indicatorsData.enquiriesCountByDate, ['date'], ['asc'])

    EnquiriesCountConfig.id = 'enquiries-count-chart_2'
    EnquiriesCountConfig.config.chart.type = 'column'
    // EnquiriesCountConfig.config.xAxis.categories = sortedData.map(item => {
    //   return '<span style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">' + item.date + '<br>' + intl.formatMessage({ id: `day.name.${DAY_NAMES[new Date(item.date).getDay()]}` }) + '</span>'
    // })
    EnquiriesCountConfig.config.xAxis.categories = sortedActiveDays.map(dayName => {
      // return '<span style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">' + dayName + '<br>' + intl.formatMessage({ id: `day.name.${DAY_NAMES[new Date(dayName).getDay()]}` }) + '</span>'
      return '<span style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">' + dayName + '</span>'
    })
    ...
    EnquiriesCountConfig.config.yAxis.title.text = intl.formatMessage({ id: 'common.home.text.0.06' })
    ...
    // EnquiriesCountConfig.config.series[0].data = sortedData.map(item => {
    //   return [item.date, item.value]
    // })
    EnquiriesCountConfig.config.series[0].data = sortedActiveDays.map(dayName => {
      let valueByDate = _.find(sortedData, { date: dayName })
  ...
  renderProceduresTopFull = () => {
    ...
    if (!this.props.indicatorsData.hasOwnProperty('tenderOfWeek')) return <Loader
      isActive={_.isEmpty(!this.props.indicatorsData.hasOwnProperty('tenderOfWeek'))}
    />

    return <ReactTable
      defaultPageSize={5}
      data={this.getProceduresTopTableData(this.props.indicatorsData.tenderOfWeek)}
  ...
  >
    <CardContentSwitch
      cardClass="switch-card-wrapper"
      childrenChart={this.renderOkbrChart()}
      // childrenInfo={<CardInfo info={CARDS_DESCRIPTIONS.topOKRB} />}
      childrenInfo={<CardInfo info={this.renderSwitchCardInfoContent(CARDS_DESCRIPTIONS.topOKRB.infoData)} />}
    ...
    <CardContentSwitch
      ...
      childrenInfo={<CardInfo info={this.renderSwitchCardInfoContent(CARDS_DESCRIPTIONS.topProcurement.infoData)} />}
    ...
    <CardContentSwitch
      ...
      childrenInfo={<CardInfo info={this.renderSwitchCardInfoContent(CARDS_DESCRIPTIONS.enquiryNumber.infoData)} />}
  ...
  renderCompetitivityDatesAmountChart = data => {
    ...
    const CHART_CONFIG = {
      ...
      yAxis: {
        title: {
          text: intl.formatMessage({ id: 'page.statistic.text.040' }),
          style: {
            color: '#AFAFAF',
            fontStyle: 'normal',
            fontWeight: 'normal',
      ...
      series: [
        {
          ...
          // data: _.map(data, 'competitiveLotsAmount'),
          data: _.map(sortedActiveDays, (dayName) => {
            let valueByDate = _.find(data, { date: dayName })
            return valueByDate ? valueByDate.competitiveLotsAmount : 0
          }),
        },
        {
          ...
          // data: _.map(data, 'notCompetitiveLotsAmount'),
          data: _.map(sortedActiveDays, (dayName) => {
            let valueByDate = _.find(data, { date: dayName })
            return valueByDate ? valueByDate.notCompetitiveLotsAmount : 0
          }),
        },
      ],
      tooltip: {
    ...
    return <div className="row margin-top-30 card-slider">
      <div className="col-md-12 left-side-wrapper">
        <Card cardClass="chart-slider-wrapper" marging={'30px 40px'} onMouseMove={this.onMouseMove}
          ...
          <Slider {...settings}>
            {!this.props.indicatorsData.hasOwnProperty('publishedLotsCount') ? <Loader
                isActive={!this.props.indicatorsData.hasOwnProperty('publishedLotsCount')}
              />
              : <div className="slider-chart">
                <div className="d-flex flex-row chart-mobile-header">
                  ...
                  <h4 className="chart-mobile-value">
                    {indicatorsData.hasOwnProperty('publishedLotsCount') && indicatorsData.publishedLotsCount + ' ' + intl.formatMessage({ id: 'common.psc.text' })}
                  </h4>
                </div>
                {
                  indicatorsData.hasOwnProperty('publishedLotsCountByDate') && this.renderKpiContractsChart(
                    indicatorsData.publishedLotsCountByDate,
                    'value',
                    false,
            ...
            {!this.props.indicatorsData.hasOwnProperty('competition') ? <Loader
                isActive={!this.props.indicatorsData.hasOwnProperty('competition')}
              />
              :
              <div className="slider-chart ">
                <div className="d-flex flex-row chart-mobile-header">
                ...
                {this.renderCompetitivityDatesAmountChart(this.props.indicatorsData.competition)}
              </div>}
          </Slider>
        </Card>
      </div>
  ...
  renderArticles = () => {
    ...
    return info.map(article => {
      return <Card key={generate()} className="col-md-12 col-xl-4 home-cards">
        <Article
          ...
          <div className="row margin-top-30">
            <div className="col-md-12">
              ...
              >
                ...
                {!this.props.indicatorsData.hasOwnProperty('topProcurements') ? <Loader
                    isActive={!this.props.indicatorsData.hasOwnProperty('topProcurements')}
                  /> :
                  <ReactTable
                    defaultPageSize={10}
...
const mapStateToProps = ({
  ...
  return {
    ...
    indicatorsDataIsFetching: indicatorsDataStore.indicatorsDataIsFetching,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    //NEW IMPORT ACTIONS
    getIndicatorsData: bindActionCreators(getIndicatorsData, dispatch),
    clearIndicatorsData: bindActionCreators(clearIndicatorsData, dispatch),
